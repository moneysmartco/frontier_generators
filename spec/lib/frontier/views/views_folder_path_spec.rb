  ...
  describe '#to_s' do
    ...
    context 'with an engine_name defined' do
      ...
      context "when there are no namespaces or nested models" do
        ...
        it { is_expected.to eq("app/views/bengine/user_documents") }
      end

      context "when there are namespaces" do
        let(:controller_prefixes) { ["admin"] }
        it { is_expected.to eq("app/views/bengine/admin/user_documents") }
      end

      context "when there are nested models" do
        let(:controller_prefixes) { ["@client"] }
        it { is_expected.to eq("app/views/bengine/client/user_documents") }
      end

      context "when there are both namespaces and nested models" do
        let(:controller_prefixes) { ["admin", "@client"] }
        it { is_expected.to eq("app/views/bengine/admin/client/user_documents") }
      end
    end
    ...
    context 'withouth an engine_name defined' do
      ...
      context "when there are no namespaces or nested models" do
        let(:controller_prefixes) { nil }
        it { is_expected.to eq("app/views/user_documents") }
      end

      context "when there are namespaces" do
        let(:controller_prefixes) { ["admin"] }
        it { is_expected.to eq("app/views/admin/user_documents") }
      end

      context "when there are nested models" do
        let(:controller_prefixes) { ["@client"] }
        it { is_expected.to eq("app/views/client/user_documents") }
      end

      context "when there are both namespaces and nested models" do
        let(:controller_prefixes) { ["admin", "@client"] }
        it { is_expected.to eq("app/views/admin/client/user_documents") }
      end
    end
  end
