...
describe Frontier::FormHeader do
  ...
  describe "#to_s" do
    ...
    context "when there are no namespaces or nested models" do
      it { is_expected.to eq("simple_form_for @model, wrapper: \"horizontal\", html: {class: \"form-horizontal\"} do |f|") }
    end

    context "when there are namespaces" do
      let(:controller_prefixes) { [:namespace] }

      it { is_expected.to eq("simple_form_for [:namespace, @model], wrapper: \"horizontal\", html: {class: \"form-horizontal\"} do |f|") }
    end

    context "when there are nested models" do
      let(:controller_prefixes) { ["@nested_model", "@other_model"] }

      it { is_expected.to eq("simple_form_for [@nested_model, @other_model, @model], wrapper: \"horizontal\", html: {class: \"form-horizontal\"} do |f|") }
    end

    context "when there are a mix of namespaces and nested models" do
      let(:controller_prefixes) { [:admin, "@nested_model", "@other_model"] }

      it { is_expected.to eq("simple_form_for [:admin, @nested_model, @other_model, @model], wrapper: \"horizontal\", html: {class: \"form-horizontal\"} do |f|") }
    end
