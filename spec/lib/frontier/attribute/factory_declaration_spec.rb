...
describe Frontier::Attribute::FactoryDeclaration do
  ...
  describe "#to_s" do
    ...
    context "attribute has an inclusion validation" do
      ...
      it { is_expected.to eq("field_name { TestModel::FIELD_NAME_VALUES.sample }")}
    end

    context "type is 'boolean'" do
      let(:type) { "boolean" }
      it { is_expected.to eq("field_name { [true, false].sample }") }
    end

    context "type is 'date'" do
      let(:type) { "date" }
      it { is_expected.to eq("field_name { 5.days.from_now.to_date }") }
    end

    context "type is 'datetime'" do
      let(:type) { "datetime" }
      it { is_expected.to eq("field_name { 5.days.from_now }") }
    end
    ...
    context "type is 'decimal'" do
      ...
      it { is_expected.to eq("field_name { some_number_value }") }
    end

    context "type is 'enum'" do
      let(:type) { "enum" }

      it { is_expected.to eq("field_name { TestModel.field_names.keys.sample }") }
    end
    ...
    context "type is 'string'" do
      ...
      it { is_expected.to eq("field_name { some_string_value }") }
    end

    context "type is 'text'" do
      let(:type) { "text" }
      it { is_expected.to eq("field_name { FFaker::Lorem.paragraph(5) }") }
    end
