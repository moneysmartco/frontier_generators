...
describe Frontier::Attribute::FeatureSpecAssignment do
  ...
  describe '#to_s' do
    ...
    context 'when boolean' do
      ...
      it { is_expected.to eq("check('Attribute name')") }
    end
    ...
    [:date, :datetime, :decimal, :integer, :string, :text].each do |attribute_type|
      context "when #{attribute_type}" do
        ...
        it { is_expected.to eq("fill_in('Attribute name', with: attributes[:attribute_name])") }
      end
    end

    context 'when enum' do
      let(:attribute_type) { :enum }
      it { is_expected.to eq("select(attributes[:attribute_name], from: 'Attribute name')") }
    end
  end
